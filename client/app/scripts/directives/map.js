...
angular.module('clientApp')
  .directive('map', function () {
    ...
    mapObj = {
        ...
        replace: true,
        template: '<form novalidate name="mapContainer" class="well mapContainer">' +
            '<div id="theMap"></div>' +
            '<div class="directions" ng-show="directions || directions==undefined">' +
            '<label>Origin:</label>' +
            '<input class="form-control" type="text" ng-model="origin" name="origin"  required>' +
            '<small class="error" id="wrongAddress">Error: \n ' +
            '<span>Sorry this is not a valid address.</span>' +
            '</small>' +
            '<label>Destination:</label>' +
            '<input class="form-control" ng-model="endPoint" type="text" disabled>' +
            '<br>' +
            '<button class="getDirections btn btn-primary" ng-click="getDirections()" ng-disabled="mapContainer.$invalid">Get Directions</button> ' +
            '<button class="clearDirections btn btn-secondary" ng-click="clearDirections()" ng-disabled="mapContainer.$invalid">Clear</button>' +
